...
- Quick setup presets for Simple, Standard, and Full damage ladders
- Thumbnail previews and per-stage token preview buttons
...
The Upload Images button lets you select several local image files at once. The module uploads them into that Actor's battle damage folder, picks a Simple, Standard, or Full setup based on image count, and assigns the images in order. Foundry's public FilePicker API does not provide safe file deletion, so Reset Stages clears the module's stage configuration but does not delete uploaded files.
